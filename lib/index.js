...
var cloudeer = {
  ...
  loadMethods: function () {
    //方法已经dump到 cloudarling，那么这里每次则不需要重新从硬盘加载方法，否则太大了。
    cloudeer.methods = [];
  ...
  saveConfig: function () {
    ...
    fs.writeFile('./data/config.json', JSON.stringify(cloudeer.config, null, "\t"), { flag: 'w' }, function (err) {
      if (err) throw err;
    });
  },
  ...
  /**
   * @deprecated 请使用 cloudoll 中相应的方法
   *
   * @param httpMethod
   * @param serviceName
   * @param methodUri
