...
var cloudeer = {
    ...
    loadConfigRemote: function (cloudeerHost) {
        ...
        share.getUrl(path.join(cloudeerHost, "/load-config"), function (err, res) {
    ...
    invoke: function (httpMethod, serviceName, methodUri, parameters, callback) {
        ...
        var accessUrl = path.join(`http://${host.host}:${host.port}`, host.baseUri, methodUri);
        ...
        if (httpMethod == "GET") {
            ...
            share.getUrl(accessUrl, function (err, body) {
                if (err) {
                    ...
                    if (jError.errno == -408) {
                        //timeout
                        share.log('Timeout, Remove the node..');
                        myHosts.splice(pickIndex, 1); //移除节点后重试
                        cloudeer.invoke(httpMethod, serviceName, methodUri, parameters, callback);

                    } else {
                        err.trace();
                        //share.log('Service error, invoke again.');
                        //出错了，不再重试，下一个版本再搞定接口级的定义
                        callback(err);
                        //if (myHosts.length > 0) {
                        //    setTimeout(function () {
                        //        cloudeer.invoke(httpMethod, serviceName, methodUri, parameters, callback);
            ...
            share.postJson(accessUrl, parameters, function (err, body) {
                if (err) {
                    //share.log(`${httpMethod} ${accessUrl} ...`);
